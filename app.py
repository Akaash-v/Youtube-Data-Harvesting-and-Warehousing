...
def list_mongodb_collection_names(database):
    akku = pymongo.MongoClient(
        "mongodb+srv://root:root@cluster0.qldsvee.mongodb.net/?retryWrites=true&w=majority")
    db = akku[database]
...
def sql_create_tables():
    akku_sql = psycopg2.connect(host='localhost', user='postgres', password='root', database='youtube')
    cursor = akku_sql.cursor()
...
def list_sql_channel_names():
    akku = psycopg2.connect(host='localhost', user='postgres', password='root', database='youtube')
    cursor = akku.cursor()
...
def sql_channel(database, col_input):
    akku_mdb = pymongo.MongoClient(
        "mongodb+srv://root:root@cluster0.qldsvee.mongodb.net/?retryWrites=true&w=majority")
    db = akku_mdb[database]
    col = db[col_input]
...
def channels_totalchannels():
    ...
    akku_s = psycopg2.connect(host='localhost', user='postgres', password='root', database='youtube')
    cursor = akku_s.cursor()
